...
impl Sample {
    pub fn new(size: usize, world: &World, policy: &ScoringPolicy, rng: &mut impl Rng) -> Sample {
        // Use the Battery's own method to get the valuation pairs
        let battery: &Battery = &world.battery;
        let observations: Vec<(Valuation, Valuation)> =
            battery.get_valuation_pairs(size, (&world.incumbent, &world.candidate), rng);

        // Compute the mean of the sample as cumulative average of discordant differences
        let mut ca: f64 = 0.0;
        let mut diff: f64;
        for i in 0..size {
            diff = utils::discord_diff(policy, &observations[i].0, &observations[i].1);
